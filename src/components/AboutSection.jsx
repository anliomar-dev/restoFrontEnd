...
function AboutSectionText() {
  return (
    <>
      <motion.p
        className="font-defaultFont text-muted text-lg"
        initial="hidden"
        whileInView="visible"
        variants={variantsSoloElement}
        viewport={{ once: true, amount: 0.5 }}
      >
        Nous vous invitons à découvrir notre carte créative, un véritable voyage
        culinaire entre tradition et modernité.
      </motion.p>
      {/*Display list of key features with icons*/}
      <motion.div
        className="about_spans flex flex-col py-5 font-defaultFont text-black text-base"
        initial="hidden"
        whileInView="visible"
      ...
      >
        <motion.span className="mb-3 flex text-base" variants={item}>
          ...
          <p className="ps-2.5 mt-1">
            Ingrédients de qualité supérieure : Nos chefs s’engagent à respecter
            la saisonnalité et le terroir.
          </p>
        </motion.span>
        <motion.span className="mb-3 flex text-base" variants={item}>
          <CircleCheck size={35} color="red" />
      ...
      <motion.p
        className="mb-5 font-defaultFont text-black text-lg"
        initial="hidden"
        whileInView="visible"
        variants={variantsSoloElement}
        viewport={{ once: true, amount: 0.5 }}
      >
        Découvrez une expérience culinaire unique où chaque plat raconte une
        histoire.
      </motion.p>
    </>
  );
...
function AboutSection() {
  return (
    <section className="flex flex-col py-16 px-8 md:px-28" id="about-section">
      <motion.div
        className="about-section_header flex flex-col justify-around items-center mb-14"
        initial="hidden"
        whileInView="visible"
        variants={containerVariants}
        viewport={{ once: true, amount: "all" }}
      >
        <motion.h2 className="text-lg text-textDefault mb-5" variants={item}>
          À propos
        </motion.h2>
        <motion.h3
          className="font-headingFont text-headingTitle text-5xl"
          variants={item}
        >
          QUI SOMME NOUS ?
        </motion.h3>
      ...
      <div className=" h-auto flex justify-center items-center lg:items-start flex-col gap-x-16 gap-y-8 lg:flex-row about-section_body ">
        ...
        >
          ...
          <motion.div
            className="border-4 border-muted mt-6 py-6 flex flex-col justify-center items-center"
            initial="hidden"
            whileInView="visible"
            variants={containerVariants}
            viewport={{ once: true, amount: 0.5 }}
          >
            <motion.h3
              className="text-headingTitle text-2xl font-bold"
              variants={item}
            >
              Réserver une table
            </motion.h3>
            <motion.p
              className="text-3xl font-bold text-accentHover"
              variants={item}
            >
              +230 5429 7857
            </motion.p>
